...
function create_menu(wobserver, additional = []){
  ...
  let header = document.createElement('header');
    ...
    header.innerHTML = '<i class="erlang-icon"></i> EMeter ';
...
const WobserverRender = {
  ...
  display_about: e => {
    ...
    let urls = about.links.map( (url) => {
      return `<tr><th>${url.name}</th><td><a href="${url.url}" target="_blank">${url.url}</a></td></tr>`
    }).join('');
    ...
    content.innerHTML = `<div class="about">
      ...
      <div class="table-holder"><table style="text-align: left;">
        <tr><th>Licence</th><td>${about.license}</td></tr>
        ${urls}
      </table></div></div>
    `;
  },
